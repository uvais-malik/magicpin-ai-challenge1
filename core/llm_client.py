...
def rewrite_reply_if_enabled(message: dict, context: dict) -> dict:
    if os.environ.get("LLM_COMPOSE", "").lower() not in {"1", "true", "yes"}:
        return message
    if message.get("action") != "send":
        return message
    ...
    system = (
        ...
        "Rewrite this bot reply to be highly specific and grounded in the current conversation context. "
        "If it's a customer booking a slot, confirm the specific time/date and update the calendar. "
        "If it's a merchant committing to an action, specify exactly what you will execute based on the trigger kind (e.g., draft a compliance checklist, launch a Diwali campaign, etc.). "
        "Keep it concise, action-oriented, and maintain the original intent. Output strict JSON with body and cta only."
    )
    prompt = json.dumps({"context": context, "base_reply": {"body": message["body"], "cta": message["cta"]}}, ensure_ascii=True)
    body = json.dumps(
        {
            "model": model,
            "temperature": 0.2,
